...
import { detectPlatform } from './platforms/common.js';
...
export class ArkeUploader {
    ...
    async uploadBatch(source, options = {}) {
        ...
        if (this.config.customPrompts) {
            ...
            const promptFields = Object.keys(this.config.customPrompts).filter(key => this.config.customPrompts[key]);
        ...
        await this.uploadFilesWithConcurrency(batch_id, files, source, this.config.parallelUploads || 5, (file, bytes) => {
            filesUploaded++;
            bytesUploaded += bytes;
            this.reportProgress(onProgress, {
                phase: 'uploading',
    ...
    async uploadSingleFile(batchId, file, source) {
        ...
        if (uploadInfo.upload_type === 'simple') {
            ...
            const parts = await uploadMultipart(fileData, partUrls, this.config.parallelParts || 3, retryOptions);
    ...
    async getFileData(file, source) {
        if (this.platform === 'node') {
            ...
            const browserFile = files.find((f) => f instanceof File && f.name === file.fileName);
            if (!browserFile) {

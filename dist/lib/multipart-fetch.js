 ...
 */
export async function uploadMultipart(fileData, presignedUrls, concurrency = 3) {
 ...
 */
async function uploadPart(partData, presignedUrl, partNumber, maxRetries = 3) {
    return retryWithBackoff(async () => {
        try {
            ...
            if (!response.ok) {
                throw new UploadError(`Part ${partNumber} upload failed with status ${response.status}: ${response.statusText}`);
            }
